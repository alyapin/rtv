...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/21 14:36:43 by kzina             #+#    #+#             */
/*   Updated: 2019/11/21 16:52:17 by kzina            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_obj(t_obj *obj, char **sl)
{
	...
	typeof_error(get_vec_data(&obj->pos, sl[1]));
	if (!sl[2])
		typeof_error(NOT_ENOUGH_DATA);
	typeof_error(get_vec_data(&rot, sl[2]));
	obj->v = vec_rot_xyz(vec_new(0, 1, 0), rot);
	if (!sl[3])
		typeof_error(NOT_ENOUGH_DATA);
	typeof_error(get_col_data(&obj->col, sl[3]));
	if (!sl[4] && !ft_strequ(sl[0], "plane:"))
		typeof_error(NOT_ENOUGH_DATA);
	if (ft_strequ(sl[0], "sphere:") || ft_strequ(sl[0], "cylinder:") ||
	ft_strequ(sl[0], "cone:"))
	{
		typeof_error(get_double(&obj->r, sl[4]));
